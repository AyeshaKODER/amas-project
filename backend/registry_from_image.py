...
class _DBRegistry:
    def __init__(self):
        ...
        try:
            ...
            # If create_all fails (db not ready), raise to trigger fallback in outer class
            raise
...
# Public AgentRegistry: use DB backend if possible, otherwise fallback
class AgentRegistry:
    def __init__(self):
        if _USE_DB:
            try:
                # attempt to initialize the DB registry (may raise if DB unreachable)
                self._impl = _DBRegistry()
                print("AgentRegistry: using Postgres-backed registry")
                return
            except Exception as e:
                print("AgentRegistry: failed to initialize DB-backed registry. Falling back to in-memory.")
                print("DB init error:", repr(e))
                # continue to fallback
        else:
            # If import failed, optionally print import exception for debugging
            try:
                print("AgentRegistry: DB modules not available; using in-memory registry.")
                if '_IMPORT_ERROR' in globals():
                    print("DB import error (truncated):")
